class SlackAPI {
    sendMessageToSlack(endPoint, message, task_id) {
        ...
        request(options, (error, response, body) => {
            if (error) {
                ...
                logger.debug(emitter.listenerCount(emitter, 'slack-message-response-200-' + task_id));
    ...
    askFirstQuestion(programmers, pms, number, task, task_id) {
        ...
        $this.sendMessageToSlack(endPoint, message, task_id);
    ...
    askQuestion(pms, number, task_id) {
        logger.debug('Ask Question ' + number);
        let endPoint = config.slack.bot.post_message_url;
        let message = '';
        let $this = this;
